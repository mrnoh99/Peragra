    ...
    static func extractPlaces(images: [(data: Data, mediaType: String)], photoKind: PhotoKind = .captionScreenshot) async throws -> [AIExtractedPlace] {
        ...
        case .onSite:
            textPrompt = images.count > 1
                ? "These photos were taken in person at a single real place — extract one consolidated, accurate result for it, cross-referencing all the photos (for example, a storefront sign for the name and a menu photo for prices/items). Only use its name if it's actually legible somewhere (a sign, menu header, receipt, storefront text, ...) — if none of these photos actually name it, use null for name rather than guessing from the type of food, decor, or general appearance; a wrong guessed name is worse than none, since it later gets treated as confirmed and searched for on a map."
                : "This photo was taken in person at a single real place (its storefront, sign, menu, or interior) — extract one accurate result for it from whatever is written or shown, such as its name and any menu items, prices, or hours visible. Only use its name if it's actually legible in the photo (a sign, menu header, receipt, storefront text, ...) — if the name isn't actually shown, use null for name rather than guessing from the type of food, decor, or general appearance; a wrong guessed name is worse than none, since it later gets treated as confirmed and searched for on a map."
        case .mapScreenshot:
