...
struct AIExtractedPlace: Decodable {
    // nil when the model can't confidently tell what this specific place is
    // called — an on-site photo with no legible sign/menu naming it, most
    // commonly. Never filled in with a guess: see the "onSite" textPrompt
    // and the shared systemPrompt's "never guess" instruction below.
    let name: String?
    let address: String?
...
enum AIExtractionService {
    ...
    - name: the place's name, exactly as written/shown — or null if you can't tell \
    with real confidence what this specific place is actually called (don't infer \
    or guess one from indirect cues like food type, decor, or general appearance)
    ...
    matching exactly this shape: {"places": [{"name": string | null, "address": string | null, \
    ...
    static func extractPlaces(images: [(data: Data, mediaType: String)], photoKind: PhotoKind = .captionScreenshot) async throws -> [AIExtractedPlace] {
        ...
        case .onSite:
            textPrompt = images.count > 1
                ? "These photos were taken in person at a single real place — extract one consolidated, accurate result for it, cross-referencing all the photos (for example, a storefront sign for the name and a menu photo for prices/items). Use whatever name-like text actually appears on a sign, storefront, menu header, or receipt — Korean and other small-business signage very often IS just a short phrase mixing a neighborhood/style and specialty (e.g. a sign reading \"신림동 명물 순대국\"), and that counts as the name even though it also describes the food; don't reject it as \"just a description.\" Only use null for name when there is truly no legible name-like text anywhere in these photos — never invent one from the food's appearance, cuisine type, or decor alone when nothing at all is written."
                : "This photo was taken in person at a single real place (its storefront, sign, menu, or interior) — extract one accurate result for it from whatever is written or shown, such as its name and any menu items, prices, or hours visible. Use whatever name-like text actually appears on a sign, storefront, menu header, or receipt — Korean and other small-business signage very often IS just a short phrase mixing a neighborhood/style and specialty (e.g. a sign reading \"신림동 명물 순대국\"), and that counts as the name even though it also describes the food; don't reject it as \"just a description.\" Only use null for name when there is truly no legible name-like text anywhere in the photo — never invent one from the food's appearance, cuisine type, or decor alone when nothing at all is written."
        case .mapScreenshot:
